...
def main(argv):
    ...
    #conf = SparkConf()
    #conf.setAppName("RPParseMsgSpark")
    #context = SparkContext(conf)
    ...
    try:
        ...
        logging.error("Could not open results file %s" % resultsFile)
        exit(1)

    outputFile.write("{messages:")
    fileCounter = 0

    testRDD = context.parallelize(decompressedDirectory)
    collectRDD1 = testRDD.collect()

# Non-spark code for reference
